...
public class NaiveClosestPair implements ClosestPairAlgorithm{
   public double findClosestPairDistance(edu.gwu.geometry.Pointd[] points){
      double mindist = findDistance(points[0], points[1]); //Find a distance to compare with new mins
      for(edu.gwu.geometry.Pointd p: points){ //Use loops to compare all points
         for(edu.gwu.geometry.Pointd d: points){
            double min = findDistance(p, d); //Find Min
            if(min < mindist && !p.equals(d)){ //If new Min
               mindist = min; //Update min
            }
         }
      }
      return mindist;
   ...
   public double findDistance(edu.gwu.geometry.Pointd one, edu.gwu.geometry.Pointd two){
      return Math.sqrt(Math.pow(two.x-one.x, 2) + Math.pow(two.y-one.y, 2)); //Use Distance Formula to Find Distance
   }
